...
class ChatRequest(BaseModel):
    ...
    mode: Optional[str] = "chat"  # NEW FIELD
...
async def chat_with_ai(request: ChatRequest):
    ...
    # ✅ Step 1: Check if it's a verify report mode first
    if request.mode == "verify_report":
        logger.info(f"Request {request_id}: Generating verify report")
